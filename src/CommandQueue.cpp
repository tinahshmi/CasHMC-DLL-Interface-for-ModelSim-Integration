...
bool CommandQueue::CmdPop(DRAMCommand **popedCMD)
{
	...
	// for (int b = 0; b < NUM_BANKS; b++)
	// {
	// 	std::cout<< "Bank "<< b << " queue=" << ACCESSQUE(b).size()<< " popDelay="<< POPCYCLE(b)<< std::endl;
	// }
	// std::cout << "issuedBank = " << issuedBank << std::endl;
	...
	//Open page
		...
		if(!sendingREForPRE) {
			...
			while(1) {
				// std::cout<< "Checking bank "<< issuedBank<< " queue="<< ACCESSQUE(issuedBank).size()<< " popDelay="<< POPCYCLE(issuedBank)
				// << std::endl;
				if(POPCYCLE(issuedBank) == 0) {
					//Issuing writing-back atomic command
					if(atomicLock[issuedBank]) {
						...
						//Search from beginning to find first issuable command
						for(int i=0; i<ACCESSQUE(issuedBank).size(); i++) {
							// std::cout<< "Trying command "<< i<< " type="<< ACCESSQUE(issuedBank)[i]->commandType<< std::endl;
							bool ok = isIssuable(ACCESSQUE(issuedBank)[i]);//tina modified
							// std::cout<< "isIssuable = "<< ok<< std::endl;
							//if(isIssuable(ACCESSQUE(issuedBank)[i])) {
								if(ok){ //tina
	...
	// cout << "CmdPop returning TRUE" << endl;
	// cout << "Returned command type = "<< (*popedCMD)->commandType<< endl;
	// cout << "Returned bank = "<< (*popedCMD)->bank<< endl;
	return true;
}
...
bool CommandQueue::isIssuable(DRAMCommand *issueCMD)
{
	switch(issueCMD->commandType) {
		case REFRESH:
			break;
		case ACTIVATE:
			cout << "------ ACTIVATE check ------" << endl;
			cout << "bank = " << issueCMD->bank << endl;
			cout << "currentClockCycle = " << currentClockCycle << endl;
			cout << "currentBankState = " << BANKSTATE(issueCMD->bank)->currentBankState << endl;
			cout << "nextActivate = " << BANKSTATE(issueCMD->bank)->nextActivate << endl;
			cout << "tFAWCountdown.size = " << tFAWCountdown.size() << endl;
			cout << "State OK = "<< (BANKSTATE(issueCMD->bank)->currentBankState == IDLE ||
					BANKSTATE(issueCMD->bank)->currentBankState == REFRESHING)<< endl;
			cout << "Time OK = "<< (BANKSTATE(issueCMD->bank)->nextActivate <= currentClockCycle)<< endl;
			cout << "tFAW OK = "<< (tFAWCountdown.size() < 4)<< endl;
			...
			&& tFAWCountdown.size() < 4) {
				//  cout << "ACTIVATE ISSUABLE" << endl;
				return true;
			}
			else {
				// cout << "ACTIVATE NOT ISSUABLE" << endl;
				return false;
			}
			break;
		case WRITE:
		case WRITE_P:
    		...
    		cout << "bank = " << issueCMD->bank << endl;
    		cout << "row = " << issueCMD->row << endl;
    		cout << "current state = "<< BANKSTATE(issueCMD->bank)->currentBankState << endl;
    		cout << "open row = "<< BANKSTATE(issueCMD->bank)->openRowAddress << endl;
    		cout << "nextWrite = "<< BANKSTATE(issueCMD->bank)->nextWrite << endl;
    		cout << "clock = "<< currentClockCycle << endl;
    		cout << "rowAccessCounter = "<< rowAccessCounter[issueCMD->bank] << endl;
    		cout << "MAX_ROW_ACCESSES = "<< MAX_ROW_ACCESSES << endl;
    		cout << "State OK = "<< (BANKSTATE(issueCMD->bank)->currentBankState == ROW_ACTIVE)<< endl;
    		cout << "Time OK = "<< (BANKSTATE(issueCMD->bank)->nextWrite <= currentClockCycle)<< endl;
    		cout << "Row OK = "<< (BANKSTATE(issueCMD->bank)->openRowAddress == issueCMD->row)<< endl;
    		cout << "Counter OK = "<< (rowAccessCounter[issueCMD->bank] < MAX_ROW_ACCESSES)<< endl;
		...
		case READ_P:
    		...
    		// cout << "current state = "<< BANKSTATE(issueCMD->bank)->currentBankState << endl;
    		// cout << "open row = "<< BANKSTATE(issueCMD->bank)->openRowAddress << endl;
    		// cout << "nextRead = "<< BANKSTATE(issueCMD->bank)->nextRead << endl;
    		// cout << "clock = "<< currentClockCycle << endl;
    		// cout << "rowAccessCounter = "<< rowAccessCounter[issueCMD->bank] << endl;
    		// cout << "MAX_ROW_ACCESSES = "<< MAX_ROW_ACCESSES << endl;
    		// cout << "State OK = "<< (BANKSTATE(issueCMD->bank)->currentBankState == ROW_ACTIVE)<< endl;
    		// cout << "Time OK = "<< (BANKSTATE(issueCMD->bank)->nextRead <= currentClockCycle)<< endl;
    		// cout << "Row OK = "<< (BANKSTATE(issueCMD->bank)->openRowAddress == issueCMD->row)<< endl;
    		// cout << "Counter OK = "<< (rowAccessCounter[issueCMD->bank] < MAX_ROW_ACCESSES)<< endl;
			if(BANKSTATE(issueCMD->bank)->currentBankState == ROW_ACTIVE
