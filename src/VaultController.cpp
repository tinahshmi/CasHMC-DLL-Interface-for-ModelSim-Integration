...
VaultController::VaultController(ofstream &debugOut_, ofstream &stateOut_, unsigned id):
	...
	vaultContID(id)
{	
	// cout << "Constructing VaultController " << this << endl; //tina
	classID << vaultContID;
...
{
	cout << "=============VaultController::ReturnCommand called=============" << endl;
	cout << "DRAM returned command" << endl;
	cout << "Type      = " << retCMD->commandType << endl;
	cout << "Bank      = " << retCMD->bank << endl;
	cout << "Row       = " << retCMD->row << endl;
	cout << "Column    = " << retCMD->column << endl;
	cout << "PacketTAG = " << retCMD->packetTAG << endl;
	cout << "DataSize  = " << retCMD->dataSize << endl;
	cout << "LastCMD   = " << retCMD->lastCMD << endl;
	cout << "Atomic    = " << retCMD->atomic << endl;
	cout << "==========================" << endl;
		...
		if(retCMD->packetTAG == pendingReadData[i]) {
			if(retCMD->lastCMD == true) {
				if(retCMD->atomic) {
					...
					// cout << endl;//tina
					cout << "Calling MakeRespondPacket()" << endl;
...
void VaultController::MakeRespondPacket(DRAMCommand *retCMD)
{
	...
	cout << "==========================" << endl;
	cout << "Inside MakeRespondPacket()" << endl;
	cout << "CommandType = " << retCMD->commandType << endl;
	cout << "packetTAG   = " << retCMD->packetTAG << endl;
	cout << "dataSize    = " << retCMD->dataSize << endl;
	cout << "==========================" << endl;
	...
	if(retCMD->atomic) {
		...
		|| retCMD->packetCMD == EQ8 || retCMD->packetCMD == EQ16 || retCMD->packetCMD == BWR) {
			...
			cout << "Response packet created: "
			<< *newPacket
			<< endl;
			cout << "TAG = " << newPacket->TAG << endl;
			cout << "LNG = " << newPacket->LNG << endl;
			cout << "CMD = " << newPacket->CMD << endl;
			pendingDataSize -= 1;
		}
		else {
	...
	//tina
	cout << "Sending packet upstream..." << endl;
	cout << "========== RESPONSE PACKET ==========" << endl;
	cout << "CMD      : " << newPacket->CMD << endl;
	cout << "TAG      : " << newPacket->TAG << endl;
	cout << "LNG      : " << newPacket->LNG << endl;
	cout << "Address  : 0x" << hex << newPacket->ADRS << dec << endl;
	cout << "Segment  : " << newPacket->segment << endl;
	cout << "=====================================" << endl;
	ReceiveUp(newPacket);
}
...
{
	//cout << "VaultController this = " << this << endl;
	//std::cout<< "VC bufPopDelay = "<< bufPopDelay << "  downBuffers = "<< downBuffers.size()<< std::endl;
	//Update DRAM state and various countdown
	UpdateCountdown();
	// std::cout << "\n===== VaultController::Update =====" << std::endl;
	// std::cout << "downBuffers.size() = "
        //   << downBuffers.size()
        //   << std::endl;
	...
	if(bufPopDelay == 0) {
		for(int i=0; i<downBuffers.size(); i++) {
			...
			// std::cout << "Trying ConvPacketIntoCMDs()..."<< std::endl;
			if(downBuffers[i] != NULL) {
				if(ConvPacketIntoCMDs(downBuffers[i])) {
					// std::cout << "ConvPacketIntoCMDs returned "<< ok << std::endl;
	...
	// std::cout << "Trying CmdPop..." << std::endl;
	// for (int b = 0; b < NUM_BANKS; b++)
	// {
    // cout << "Bank " << b<< " state=" << dramP->bankStates[b]->currentBankState<< endl;
	// }
	bool ok = commandQueue->CmdPop(&poppedCMD);
	// cout << "CmdPop() returned "<< ok << endl;
	//if(commandQueue->CmdPop(&poppedCMD)) { //tina
	if(ok){ //tina
		// cout << "poppedCMD = "<< poppedCMD<< endl;
		// cout << "type = "<< poppedCMD->commandType<< endl;
		...
		cmdBus = poppedCMD;
		cmdCyclesLeft = tCMD;
		// cout << "cmdBus assigned" << endl;
		// cout << "cmdCyclesLeft = "<< cmdCyclesLeft<< endl;
		poppedCMD = NULL;
	}else
	{
		// std::cout << "CmdPop returned FALSE" << std::endl;
	}
...
{
	// cout << "UpdateCountdown this = " << this << endl;
	// cout << "\n=== UpdateCountdown ===" << endl;
	// cout << "this = " << this << endl;
	// cout << "cmdBus = " << cmdBus << endl;
	// cout << "cmdCyclesLeft = " << cmdCyclesLeft << endl;
	//Check for outgoing command and handle countdowns
	if(cmdBus != NULL) {
		// cout << "cmdBus exists" << endl;

		cmdCyclesLeft--;
		// cout << "after decrement = "<< cmdCyclesLeft<< endl;
...
{
	// std::cout << "Entering ConvPacketIntoCMDs()" << std::endl;
	// std::cout << "QUE_PER_BANK = " << QUE_PER_BANK << std::endl;
	// std::cout << "bankAdd = " << bankAdd << std::endl;
	...
	if(commandQueue->AvailableSpace(bankAdd, ceil((double)packet->reqDataSize/32)+1)) {
		unsigned need = ceil((double)packet->reqDataSize/32)+1;
		// std::cout << "bankAdd = " << bankAdd << std::endl;
		// std::cout << "need commands = " << need << std::endl;
		bool ok = commandQueue->AvailableSpace(bankAdd, need);
		// std::cout << "AvailableSpace = " << ok << std::endl;
		if(ok){std::cout << "Enqueue ACT" << std::endl;}
		...
		// std::cout << "ACT enqueued" << std::endl;
		for(int i=0; i<ceil((double)packet->reqDataSize/32); i++) {
			// std::cout<< "Enqueue RW bank="<< bankAdd<< " last="<< rwCMD->lastCMD<< std::endl;
			// std::cout << "RW enqueued " << i << std::endl;
			DRAMCommand *rwCMD;
		...
		// std::cout << "Finished enqueueing commands." << std::endl;
		return true;
	}
	else{
		return false;
...
{
	// cout << "\n=== EnablePowerdown ===" << endl;
    // cout << "USE_LOW_POWER = " << USE_LOW_POWER << endl;
    // cout << "queue empty = " << commandQueue->isEmpty() << endl;
    // cout << "refreshWaiting = " << commandQueue->refreshWaiting << endl;
    // cout << "powerDown flag = " << powerDown << endl;
    // cout << "bank0 state = " << dramP->bankStates[0]->currentBankState << endl;
    // cout << "nextPowerUp = " << dramP->bankStates[0]->nextPowerUp << endl;
    // cout << "clock = " << currentClockCycle << endl;
	if(USE_LOW_POWER) {
		// cout << "EnablePowerdown:" << endl;
		// cout << "queueEmpty = " << commandQueue->isEmpty() << endl;
		// cout << "refreshWaiting = " << commandQueue->refreshWaiting << endl;
		// cout << "powerDown = " << powerDown << endl;
		// cout << "nextPowerUp = " << dramP->bankStates[0]->nextPowerUp << endl;
		// cout << "currentClock = " << currentClockCycle << endl;
		if(commandQueue->isEmpty() && !commandQueue->refreshWaiting) {
			//cout << ">>> CALLING POWERUP" << endl;
			if(dramP->powerDown()) {
				//cout << "Entering POWERDOWN" << endl;
				powerDown = true;
			}
		}
		else if(powerDown && dramP->bankStates[0]->nextPowerUp <= currentClockCycle) {
			dramP->powerUp();
			// cout << "POWERUP CALLED" << endl;
			powerDown = false;
		}
	}
}
